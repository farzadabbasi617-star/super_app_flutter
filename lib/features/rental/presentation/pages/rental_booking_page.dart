...
import 'package:super_app_flutter/features/marketplace/domain/entities/product.dart';
...
class _RentalBookingPageState extends State<RentalBookingPage> {
        ...
        if (isStart) {
          ...
          if (picked.isAfter(_startDate) ||
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('تاریخ پایان باید بعد از تاریخ شروع باشد!'),
  ...
  Widget build(BuildContext context) {
    ...
    final formattedPricePerDay =
        widget.product.rentalPricePerDay.toStringAsFixed(0).replaceAllMapped(
              RegExp(r"(\d{1,3})(?=(\d{3})+(?!\d))"),
              (Match m) => "${m[1]},",
            );

    final formattedTotalRent =
        totalRentCost.toStringAsFixed(0).replaceAllMapped(
              RegExp(r"(\d{1,3})(?=(\d{3})+(?!\d))"),
              (Match m) => "${m[1]},",
            );

    final formattedDeposit =
        securityDeposit.toStringAsFixed(0).replaceAllMapped(
              RegExp(r"(\d{1,3})(?=(\d{3})+(?!\d))"),
              (Match m) => "${m[1]},",
            );

    final formattedGrandTotal = grandTotal.toStringAsFixed(0).replaceAllMapped(
          RegExp(r"(\d{1,3})(?=(\d{3})+(?!\d))"),
          (Match m) => "${m[1]},",
        );
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          'ثبت درخواست و اجاره ماشین‌آلات',
              ...
              Row(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      widget.product.category == 'Industrial'
                          ? 'صنعتی'
                          : 'کشاورزی',
                      style: TextStyle(
              ...
              Container(
                ...
                child: Column(
                  children: [
                    _buildCalcRow(
                      'تعرفه اجاره روزانه:',
                      '$formattedPricePerDay تومان / روز',
                    ),
                    ...
                    _buildCalcRow(
                      'مجموع هزینه اجاره:',
                      '$formattedTotalRent تومان',
                    ),
                    const SizedBox(height: 6),
                    _buildCalcRow(
                      'وثیقه و سپرده سلامت (۲۰٪):',
                      '$formattedDeposit تومان',
                    ),
                    const Divider(height: 20),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  ...
                  onPressed: () {
                    ...
                    if (_monetization.walletBalance >= grandTotal) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            '❌ موجودی کیف پول کافی نیست! ابتدا حساب خود را شارژ کنید.',
                          ),
                          backgroundColor: Colors.red,
                        ),
                      );
