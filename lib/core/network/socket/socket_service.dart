...
class SocketService {
  ...
  void initSocket(String token) {
    _socket = IO.io('https://api.superapp.com/socket', 
      IO.OptionBuilder()
        ...
        .setAuth({'token': token})
        .setReconnection(false) // Handle reconnection manually with Exponential Backoff!
        .build());
    ...
    _socket!.onDisconnect((_) {
      AppLogger.log('Socket Disconnected: Starting Exponential Backoff', level: LogLevel.warning);
      _stopHeartbeat();
  ...
  void _triggerBackoffReconnection(String token) {
    ...
    AppLogger.log('Socket: Attempting reconnection #$_reconnectAttempts in $delaySeconds seconds...', level: LogLevel.info);
